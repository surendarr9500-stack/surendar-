# Offline & Sync Plan - Capacity Connect
...
- Stream: connectivityStream → UI banner OFFLINE/ONLINE
...
- Progress tracking, settings, storage management, audit logging local

## Local AI Offline
- Engine runs on 127.0.0.1:8001, no internet needed
- Knowledge base pre-bundled in app assets + downloadable updates
- Fallback: if AI engine down, use Dart-side deterministic keyword matcher (same logic ported)

## Sync Queue Design
Table: sync_queue
- transaction_id UUID PK, device_id, user_id, entity_type, entity_id, operation, payload JSON, created_at, updated_at, sync_status ENUM, retry_count, version, error_message

Every local write:
```dart
await db.transaction(() async {
  await dao.updateEntity(entity);
  await syncQueueDao.enqueue(entity_type: 'diagnostic', entity_id: entity.id, operation: 'UPDATE', payload: entity.toJson(), version: entity.version);
});
```
...
Retry: Exponential backoff 1s,2s,4s,8s,16s, max 5 retries, then FAILED. User can manually retry from Sync Status page.

Batching: 50 per batch to avoid timeout. Attachments uploaded separately via multipart.

## Conflict Resolution Strategies per Entity
- diagnostics: field-level merge, never silent loss, preserve technician notes, last-write-wins for status only if newer timestamp but preserve history
- maintenance_records: version compare, manual resolution for critical
- progress, quiz_attempts: last-write-wins with version (higher wins, but preserve offline quiz completion)
...
Server stores both client and server payload in sync_transactions.conflict_data. Client SyncConflictResolver UI shows side-by-side diff. Resolution via POST /api/v1/sync/resolve-conflict.
...
- GlobalSearchService aggregates from multiple FTS tables
...
- Backup: copy SQLite file to backup dir before major operations, keep 3

## Failure Handling
- Internet lost during sync: mark SYNCING→PENDING, retry later
- App killed during sync: on next launch, reset SYNCING to PENDING
- DB error: error screen, attempt recovery from backup
- Corrupted asset: placeholder, allow re-download
- AI unavailable: fallback to Dart matcher
- 3D model missing: component data list view

## Audit & Observability Offline
...
- Integration: simulate offline via mock connectivity, verify writes still work, then online and verify sync
- E2E: Full flow disconnect, troubleshooting, diagnostic create, quiz, restart, reconnect, sync
- Failure: kill app during sync, restart device offline, corrupted DB, invalid API response

## UI/UX Offline
- Banner: OFFLINE - LOCAL ENGINE ACTIVE
- Sync page: pending count, last sync, retry button, conflict list
- Dashboard: network, storage, security, training %, alerts, twin health, sync queue
